...
def Create_full_dataset(dataset_name='Lizas_Dataset_1',
    ...
    # get the PV data 
    historical_load = _get_PV(pv_data_path, start_end_date)

    faults = np.zeros([weather_nwp.shape[0],1]) # array describing the faults in the training data
    # Scale the data
    scaler_weather, scaler_load = StandardScaler(), StandardScaler()
    scaler_weather.fit(weather_nwp)
    scaler_load.fit(historical_load.reshape(len(historical_load),1))
    weather_nwp = scaler_weather.transform(weather_nwp)
    historical_load = scaler_load.transform(historical_load.reshape(len(historical_load),1))


    history_shape = historical_load.shape
    history_min_max = [np.amin(historical_load), np.amax(historical_load)]
    ...
    target_downsampling_rate = int(target_len_samples / target_steps)
    nwp_downsampling_rate = 1
    ...
    def __save_set_to_folder(sub_dataset, folder_name='train', save_every_xth_step=1):
        ...
        for sample_index in sub_dataset:
            ...
            history_support = historical_load[sample_index:(sample_index+sw_len_samples)]

            target_start_index = sample_index + sw_len_samples
            target = historical_load[target_start_index:(target_start_index+fc_len_samples)]

            teacher_start_index = target_start_index - one_addl_fc_timestep
            teacher = historical_load[teacher_start_index:(teacher_start_index+fc_len_samples)]

            persistency_start_index = target_start_index - fc_len_samples
            persistency = historical_load[persistency_start_index:(persistency_start_index+fc_len_samples)]
        ...
        os.chdir(dataset_folder_path)
        if not partials:
            sample_baseline = __calculatate_skillscore_baseline(target_pdf, persistent_forecast=persistency_pdf, normalizer_value=np.amax(historical_load))
        else:
            sample_baseline = {
    ...
    dataset_info['normalizer_value'] = np.amax(historical_load) - np.amin(historical_load)
...
def _get_weather_data(weather_data_folder):
    ...
    #         buffer.append(str(date) +' '+ str(hour)+':'+str(min)+':0')

    buffer_year = []
    h = 0
    last_date = '2012-12-31'
    for date, hour in zip(read_csv[0]['Date'], read_csv[0]['Hour']):
        if date[0:4] == last_date[0:4]:
            h = h + 1.0
        else:
            h = 0.0
        for min in range(4):
            if min == 0:
                buffer_year.append(h)
            else:
                buffer_year.append(h + min/4)
        last_date = date

    buffer_day = []
    for hour in read_csv[0]['Hour']:
        for min in range(4):
            if min == 0:
                buffer_day.append(float(hour))
            else:
                buffer_day.append(float(hour) + min/4)


    X_weather = np.concatenate((np.expand_dims(buffer_day,1), np.expand_dims(buffer_year,1), X_weather), -1)

    weather_mvts = X_weather
    del X_old, X_weather

    print('converting wind to polar coordinates')
    wind = weather_mvts[:,-1]
    weather_mvts = np.delete(weather_mvts, -1, axis=-1)
    # *10 because the wind direction is given in 10s of degrees
    wind = wind*10.0
    ...
    datetime_array_daytime = weather_mvts[:,0]
    ...
    datetime_array_yeartime = weather_mvts[:,0]
    weather_mvts = np.delete(weather_mvts, 0, axis=-1)
    ...
    start_nwp = read_csv[0]['Date'][0]
    end_ind = len(read_csv[0])
    end_nwp = read_csv[0]['Date'][int(end_ind-1)]
...
def _get_PV(path, start_end_date): #getting the PV data
    ...
    for i in range(2013,2019):
        ...
        if i > 2013:
            ...
            X_load = f
    # subtract data points after 08-16-2018 because the weather data is missing (add later)
    X_load = X_load[:197184] # this number is the shape of the weather data
...
def __create_and_save_1_fold_CrossValidation_Liza(name='Lizas_dataset_3', seeds=[43, 42]):
